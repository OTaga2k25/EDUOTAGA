...
function getCodeLines() {
  ...
  return [
    ...
    {n:2, html:`<span class="pl">&nbsp;&nbsp;new_s = </span><span class="str">""</span>`},
    {n:3, html:`<span class="pl">&nbsp;&nbsp;</span><span class="kw">for</span><span class="pl"> i </span><span class="kw">in</span><span class="pl"> </span><span class="fn">range</span><span class="pl">(</span><span class="fn">len</span><span class="pl">(s) - </span><span class="num">1</span><span class="pl">, -</span><span class="num">1</span><span class="pl">, -</span><span class="num">1</span><span class="pl">):</span>`},
    {n:4, html:`<span class="pl">&nbsp;&nbsp;&nbsp;&nbsp;new_s </span><span class="op">+=</span><span class="pl"> s[i]</span>`},
    {n:5, html:`<span class="pl">&nbsp;&nbsp;</span><span class="kw">return</span><span class="pl"> new_s</span>`},
    {n:'&nbsp;', html:``},
    {n:'&nbsp;', html:`<span class="pl">my_word = </span><span class="str">"${rawWord}"</span>`},
    {n:'&nbsp;', html:`<span class="fn">reverse_string</span><span class="pl">(my_word)</span>`}
  ];
}
function renderCode(activeLine, stepType, sObj){
  ...
  switch(activeLine) {
    case 1: 
      text = 'Start the function passing the immutable string we want to reverse.'; 
      break;
    case 2:
      text = 'Allocate a completely new, empty string in memory to hold our result.';
      break;
    case 3:
      if (rChar) text = `Start a pointer at the end of the string (at '${rChar}') and iterate backwards.`;
      else text = 'Start a pointer at the very end of the string and iterate backwards to the beginning.';
      break;
    case 4:
      if (lChar || rChar) text = `Allocate new memory and append '${lChar || rChar}' to the new string.`;
      else text = 'Allocate memory to append the current character to the new string.';
      break;
    case 5:
      text = 'Hand back the brand new string — allocating O(N) extra space in total!';
      break;
    default:
      text = 'The algorithm finishes building the new string.';
  }
  pt.textContent = text;
}
const CONTENT = {
...
if(btnLoadCustom && customWordInput) {
  btnLoadCustom.addEventListener('click', () => {
    ...
    if (subtitle) {
      subtitle.innerHTML = `"${rawWord}" &rarr; "${reversed}" &middot; iterate backwards to build a new string`;
    }
